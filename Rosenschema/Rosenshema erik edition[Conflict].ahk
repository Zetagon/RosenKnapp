...
SysGet, Mon1, MonitorWorkArea ;NYYYYYYYYYTTTTT Figuring out the monitor resolution minus taskbar to be able to request the corret schedule dimensions.
WorkWidth := Mon1Right
WorkHeight := Mon1Bottom
...
loop, read, C:\Users\leooka1ros\Google Drive\Skript\Rosenschema\id	;puts every line of the file into Array
{
	Array%A_index% := A_LoopReadLine
	Array0 = %A_Index%
}
...
SchemaWidth := (WorkWidth-3) ;adjusting size to fit the whole schedule inside the GUI window.
SchemaHeight := (WorkHeight-41)
UrlDownloadToFile, http://www.novasoftware.se/ImgGen/schedulegenerator.aspx?format=png&schoolid=81320/sv-se&type=-1&id=%schemaid%&period=&week=14&mode=0&printer=0&colors=2&head=1&clock=1&foot=1&day=0&width=%SchemaWidth%&height=%SchemaHeight%&count=1&decrypt=0, Schema.png
Gui, Add, Picture,x0,Schema.png
Gui, Add, text, xCenter, Tryck "alt Tab" för att gå ur schemat!
